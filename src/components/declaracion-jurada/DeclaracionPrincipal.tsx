...
import { SeccionesInformativas } from "../secciones/seccion1";
...
export default function DeclaracionPrincipal() {
  ...
  return (
    <>
      {/* SECCIÓN PRINCIPAL - Hero con declaración */}
      <div
        className="relative w-screen min-h-[120vh] overflow-hidden transition-all duration-700"
        style={{
          background: isDarkMode
            ? "linear-gradient(135deg, #1e3a8a 0%, #0f172a 50%, #1e3a8a 100%)"
            : "linear-gradient(135deg, #F5F9FF 0%, #F5F9FF 50%, #F5F9FF 100%)",
        }}
      >
        ...
        <div className="hidden lg:block absolute right-0 bottom-[6vh] z-10 w-[55vw] h-[95vh] pointer-events-none">
          <RandomPerson />
        </div>
      </div>

      {/* SECCIÓN 1 - Se renderiza debajo del hero */}
      <SeccionesInformativas />

      {/* Estilos CSS globales - MODIFICADOS para permitir scroll */}
      <style jsx global>{`
        html, body {
